    ...
    pub(crate) fn from_nz_u32(errno: NonZeroU32) -> Self {
        ...
        if code == nng_err::NNG_ECANCELED as u32 {
            ...
            let err = match code {
                ...
                c if (c & (nng_err::NNG_ESYSERR as u32)) != 0 => {
                    let sys_err = (code & !(nng_err::NNG_ESYSERR as u32)) as i32;
                    let kind = io::Error::from_raw_os_error(sys_err).kind();
                    return AioError::Operation(NngError::System(kind));
                }
                c if (c & (nng_err::NNG_ETRANERR as u32)) != 0 => {
                    let tran_err = code & !(nng_err::NNG_ETRANERR as u32);
                    return AioError::Operation(NngError::Transport(tran_err));
                }
                _ => return AioError::Operation(NngError::Other(errno)),
            };
            AioError::Operation(NngError::NngError(err))
        }
    }
...
impl NngError {
    ...
    ///
    /// This method handles both legacy and typed error representations,
    /// comparing the underlying error code in either case.
    pub fn is(&self, err: nng_err) -> bool {
        match self {
            NngError::NngError(e) => *e == err,
            NngError::Transport(_) | NngError::System(_) => false,
            NngError::Other(code) => code.get() == err as u32,
        }
    }
}
